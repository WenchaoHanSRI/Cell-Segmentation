...
def loop_objects_val_objects(image_seg, image_name_val, ROIsize):
    ...
    Dict_EM = Parallel(n_jobs=24)(delayed(compute_instance_EM)(image_mask1, i, image_name_val, ROIsize) for i in inputs)
    return Dict_EM
...
def computeErrorMetricsMask(ExpName, Path_Seg, Path_GT, filename, ROIsize):
    ...
    EM = [f1, kappa, h, f1_pixel, ac_pixel]
...
# ExpName = 'FLTrans2048ROI1MINIMask'
ROIsize = 2048
path_current = os.path.dirname(os.path.realpath(__file__))
Path_GT = os.path.join(args.dataset, 'GTPyIdx')
# infer_result_path = os.path.join(outputpath,'Infer_results')
# infer_result_path = os.path.join(result_path, ExpName)
